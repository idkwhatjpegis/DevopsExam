...
# Add user
docker container exec -u 1000 gitea gitea admin user create --admin --username vagrant --password vagrant --email vagrant@do1.exam  --must-change-password=false

# Clone the project
git clone https://github.com/shekeriev/fun-facts /tmp/exam
# Add it to Gitea (but as public repository)
sudo echo "Take">> /tmp/exam/takeexam.txt
cd /tmp/exam &&  git add . && git commit -m "Modified on $(date '+%Y-%m-%d %H:%M:%S')" && git push -o repo.private=false http://vagrant:vagrant@192.168.111.101:3000/vagrant/exam
# Add a Webhook
